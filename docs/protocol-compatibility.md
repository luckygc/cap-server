...
`f9ffadb`）之间的 wire 约定。公开 Java API 使用 records 表示这些字段，但宿主应用负责 HTTP 和
JSON 序列化。
...
统一兑换请求对应 `RedeemRequest(token, solutions, instr, instrBlocked, instrTimeout)`：

- Format 1：`solutions` 是与 `c` 等长的整数数组；启用 instrumentation 时，顶层 `instr` 包含
  `i`、`state`、可空 `ts`，顶层 `instrBlocked` / `instrTimeout` 传递浏览器信号。
...
成功响应 `RedeemResult.Success` 的字段是 `success=true`、`token`、可空 `tokenKey`、`expires`、
可空 `scope`、`iat`。失败响应 `RedeemResult.Failure` 的字段是 `success=false`、`reason`、
`instrError`、可空 `error`；当前默认实现不把内部异常消息放入 `error`。
...
instrumentation 类失败的 `instrError` 为 `true`；其他失败为 `false`。Format 2 已通过 JWT 但
缺失或结构损坏的认证元数据统一映射为 `invalid_token`。
...
2. 直接暴露 `ChallengeResponse`、`RedeemRequest`、`RedeemResult`；
